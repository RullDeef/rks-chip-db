...
public class Fouterdbrefuse extends AbstractFrame implements TyponominalFrame {
    ...
    @Inject
    private CollectionDatasource<OuterDbRefuse, UUID> outerDbRefuseDs;
    @Inject
    private Table<OuterDbRefuse> tblOuterDbRefuse;
    @Inject
    ...
    public void onBtnCreateClick() {
        ...
        AbstractEditor ed = openEditor("mobdekbkp$OuterDbRefuse.edit", newItem, WindowManager.OpenType.DIALOG, paramOpen);
    ...
    public void onEdit(Component source) {
        Map<String, Object> paramOpen = new HashMap<>();
        paramOpen.put("noedit", true);
        AbstractEditor ed = openEditor("mobdekbkp$OuterDbRefuse.edit", outerDbRefuseDs.getItem(), WindowManager.OpenType.DIALOG, paramOpen);
        ed.addCloseWithCommitListener(() -> thisframe.initFrame(tn));
    }
